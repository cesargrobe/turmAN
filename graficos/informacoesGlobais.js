...
async function visualizarInformacoesGlobais() {
    ...
    paragrafo.innerHTML = `O trabalho agrega valor e propósito à vida das pessoas. Atualmente existem cerca de <span>${total_pessoas_mundo.toFixed(2)}</span> bilhões de pessoas no mundo, destes, cerca de 2 bilhões de pessoas precisam de um meio de transporte para chegar ao seu trabalho levando em média <span>${tempo_medio_deslocamento_para_trabalho.toFixed(2)} </span> minutos. <br> <br> 
    
    Isso significa que aproximadamente <span>${total_pessoas_que_necessitam_transporte_para_trabalho}</span> das pessoas dependem de algum meio de transporte para trabalhar. Considerando a média de tempo para chegar ao trabalho, cerca de 13% do tempo diário é passado em um meio de transporte. <br> <br> 
